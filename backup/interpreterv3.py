...
class Interpreter(InterpreterBase):
    ...
    def __run_statements(self, statements, lambda_env = None, func_name = None):
        ...
        for statement in statements:
            ...
            if lambda_env is not None and func_name is not None:
                ...
                find = self.env.get(func_name)
                self.env.set(func_name, Value(Type.FUNCTION, {"name": find.value().get("name"), "env": updated_lambda_env, "params": find.value().get("params"), "body": find.value().get("body")}))
    ...
    def __eval_expr(self, expr_ast):
        ...
        if expr_ast.elem_type == Interpreter.LAMBDA_DEF:
            ...
            val = FunctionNode(self.env.flatten_env_to_dict(), expr_ast.get("args"), expr_ast, lambda_name, Type.LAMBDA)
            ...
            if func_name not in self.func_name_to_ast:
                self.func_name_to_ast[func_name] = {}
            self.func_name_to_ast[func_name][num_params] = func_def
            return Value(Type.REF, {"elem_type": "lambda", "name": lambda_name})
